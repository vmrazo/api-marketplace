...
//    Peticiones GET para el buscador entre tablas relacionadas
    static public function getSearchRelData($rel,$type,$linkTo,$search,$orderBy,$orderMode,$startAt,$endAt){
        ...
        if(count($relArray) == 3 && count($typeArray) == 3){
            ...
            if ($orderBy != null && $orderMode != null && $startAt == null && $endAt == null){
                $stmt = Connection::connect()->prepare("SELECT * FROM $relArray[0] inner join $relArray[1] ON $on1a = $on1b INNER JOIN
                    $relArray[2] ON $on2a = $on2b WHERE $linkTo LIKE '%$search%' ORDER BY $orderBy $orderMode");
            }elseif($orderBy != null && $orderMode != null && $startAt != null && $endAt != null){
                $stmt = Connection::connect()->prepare("SELECT * FROM $relArray[0] inner join $relArray[1] ON $on1a = $on1b INNER JOIN
                    $relArray[2] ON $on2a = $on2b WHERE $linkTo LIKE '%$search%' ORDER BY $orderBy $orderMode LIMIT $startAt, $endAt");
            }
            else{
                $stmt = Connection::connect()->prepare("SELECT * FROM $relArray[0] inner join $relArray[1] ON $on1a = $on1b INNER JOIN
                    $relArray[2] ON $on2a = $on2b WHERE $linkTo LIKE '%$search%'");
            }
        ...
        if(count($relArray) == 4 && count($typeArray) == 4){
            ...
            if ($orderBy != null && $orderMode != null && $startAt == null && $endAt == null){
                $stmt = Connection::connect()->prepare("SELECT * FROM $relArray[0] inner join $relArray[1] ON $on1a = $on1b INNER JOIN
                    $relArray[2] ON $on2a = $on2b INNER JOIN $relArray[3] ON $on3a = $on3b WHERE $linkTo LIKE '%$search%' ORDER BY $orderBy $orderMode");

            }elseif($orderBy != null && $orderMode != null && $startAt != null && $endAt != null){
                $stmt = Connection::connect()->prepare("SELECT * FROM $relArray[0] inner join $relArray[1] ON $on1a = $on1b INNER JOIN
                    $relArray[2] ON $on2a = $on2b INNER JOIN $relArray[3] ON $on3a = $on3b WHERE $linkTo LIKE '%$search%' ORDER BY $orderBy $orderMode LIMIT $startAt, $endAt");
            }
            else{
                $stmt = Connection::connect()->prepare("SELECT * FROM $relArray[0] inner join $relArray[1] ON $on1a = $on1b INNER JOIN
                    $relArray[2] ON $on2a = $on2b INNER JOIN $relArray[3] ON $on3a = $on3b WHERE $linkTo LIKE '%$search%'");
